...
실제 백엔드가 없는 프로젝트라, 포트폴리오용으로 배포할 때도 화면이 실제로 동작하는 걸
보여주려면 목업을 켠 채로 배포해야 한다. 다만 진짜 백엔드가 생겼을 때를 대비해 **기본값은
여전히 "프로덕션 빌드에 목업 제외"**를 유지하고, 데모 배포에서만 명시적으로 켜는 방식으로
만들었다.

- Vercel 프로젝트 환경변수에 `VITE_ENABLE_MOCKS=true`만 추가하면 된다
  (`VITE_API_BASE_URL`은 비워둔다 — 같은 origin으로 요청하고 MSW가 그대로 가로챈다).
- 이 값은 [MOCKS_ENABLED](src/mocks/isEnabled.ts)라는 하나의 상수로 모아뒀고,
  목업 활성화 여부에 따라 갈리는 화면(로그인 화면의 테스트 계정 안내, 배송 자동 진행
  시뮬레이션)도 전부 이 값을 함께 쓴다 — "DEV냐 아니냐"가 아니라 "목업으로 동작 중이냐"가
  진짜 기준이기 때문이다.
- SPA 라우팅(예: `/orders/1/delivery`를 새로고침하거나 직접 열었을 때)이 404가 나지 않도록
  [vercel.json](vercel.json)에 전체 경로를 `index.html`로 돌리는 rewrite를 추가했다.
- 배포 전 로컬에서 실제 프로덕션 빌드로 동일하게 검증했다:
  `VITE_ENABLE_MOCKS=true npx vite build` 후 `npx vite preview`로 로그인부터 배송 상태
  변경까지 전부 확인.
- 이 작업 중 실제 버그 하나를 고쳤다: `main.tsx`가 목업 초기화 실패 시(`.then()`만 있고
  `.catch()`가 없어서) 화면이 영원히 빈 채로 남는 문제가 있었다 — 목업이 꺼져 있거나
  초기화에 실패해도 앱은 항상 렌더링되도록 고쳤다.
